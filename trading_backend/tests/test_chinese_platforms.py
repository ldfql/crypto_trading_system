        ...
        try:
            async with asyncio.timeout(120):  # Overall test timeout
                for case in test_cases:
                    ...
                    sentiment, confidence = await chinese_scraper.analyze_sentiment(case['text'])

                    logger.info(f"Got sentiment score: {sentiment}, confidence: {confidence}")
                    ...
                    assert abs(sentiment - case['expected_sentiment']) <= 0.3, \
