...
def mock_youtube_client(monkeypatch):
    ...
    def mock_build(*args, **kwargs):
        ...
        mock_client.channels().list().execute.return_value = {
            "items": [
                {"id": "test_channel_id", "statistics": {"subscriberCount": "100000"}}
            ]
        }
...
def create_mock_video(
    title: str, description: str, views: int, likes: int, hours_ago: int = 1
):
    return {
        "id": "video123",
        "snippet": {
            ...
            "publishedAt": (datetime.now(UTC) - timedelta(hours=hours_ago)).strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            ),
        },
...
async def test_get_trading_insights(youtube_scraper, mock_youtube_client):
    # Mock responses
    channel_response = {
        "items": [{"id": "channel123", "statistics": {"subscriberCount": "100000"}}]
    }
...
async def test_get_strategy_insights(youtube_scraper, mock_youtube_client):
    ...
    videos_response = {
        "items": [
            create_mock_video(
                "Crypto Trading Signals", "Market analysis", 100000, 10000
            )
        ]
    }
...
async def test_api_quota_exceeded(youtube_scraper, mock_youtube_client):
    ...
    mock_client.channels().list().execute.side_effect = HttpError(
        error_response, b"Quota exceeded"
    )
...
def test_is_trading_related(youtube_scraper):
    ...
    non_trading_snippet = {"title": "My Daily Vlog", "description": "A day in my life"}
...
def test_calculate_influence_weight(youtube_scraper):
    ...
    low_weight = youtube_scraper._calculate_influence_weight(
        views=100, likes=10, subscribers=1000000, comments=5
    )
