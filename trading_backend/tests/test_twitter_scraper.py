...
def create_mock_tweet(
    text: str, likes: int, retweets: int, followers: int, hours_ago: int = 1
):
...
async def test_get_influential_tweets(twitter_scraper, mock_tweepy_api):
    ...
    tweets_account1 = [
        ...
        create_mock_tweet(
            "Just had coffee! Great day!", 100, 50, 10000  # Non-trading tweet
        ),
    ]
    ...
    tweets_account2 = [
        create_mock_tweet(
            "ETH breaking resistance, time to long! #ethereum", 2000, 1000, 100000
        ),
        create_mock_tweet(
            "Beautiful weather today!", 200, 100, 20000  # Non-trading tweet
        ),
    ]
