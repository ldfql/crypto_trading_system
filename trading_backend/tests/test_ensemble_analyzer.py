...
@pytest.mark.parametrize("text,expected_sentiment", [
    ...
    ("Death cross forming with heavy distribution and bearish divergence", "bearish"),
    ("Market consolidating in range with mixed signals from institutions", "neutral"),
])
@pytest.mark.asyncio
...
@pytest.mark.parametrize("text,expected_sentiment", [
    ("Institutional investors increasing Bitcoin holdings significantly", "bullish"),
...
async def test_error_handling():
    analyzer = EnsembleSentimentAnalyzer()
    with patch.object(analyzer, '_get_bert_sentiment', side_effect=Exception("BERT error")):
...
async def test_confidence_calculation():
    ...
    with patch.object(analyzer, '_get_bert_sentiment', return_value=("bullish", 0.9)), \
         patch.object(analyzer, '_apply_technical_rules', return_value=("bullish", 0.85)), \
         patch.object(analyzer, '_analyze_market_context', return_value=("bullish", 0.88)):
        result = await analyzer.analyze_sentiment("test text")
        assert result['confidence'] > 0.85

    with patch.object(analyzer, '_get_bert_sentiment', return_value=("bullish", 0.4)), \
         patch.object(analyzer, '_apply_technical_rules', return_value=("bearish", 0.35)), \
         patch.object(analyzer, '_analyze_market_context', return_value=("neutral", 0.33)):
        result = await analyzer.analyze_sentiment("test text")
