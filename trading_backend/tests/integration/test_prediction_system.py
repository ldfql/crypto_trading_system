...
async def sample_signals(signal_repository):
    ...
    for i in range(100):
        signal_data = {
            ...
            'validation_count': 10,
            'accuracy': 0.85 + (i % 10) * 0.01 if i < 80 else None  # Leave some signals without accuracy
        }
...
async def test_prediction_analysis_and_improvement(
    prediction_analyzer,
    sample_signals
):
...
async def test_accuracy_requirements(
    prediction_analyzer,
    sample_signals
):
    ...
    if accuracies:
        ...
        assert average_accuracy >= 0.85, f"Average accuracy {average_accuracy} is below 85% requirement"

async def test_historical_comparison(
    prediction_analyzer,
    sample_signals
):
...
async def test_continuous_improvement(
    ...
    improvement_report = await prediction_analyzer.generate_strategy_improvement_report(days=60)
...
async def test_market_phase_adaptation(
    prediction_analyzer,
    sample_signals
):
...
async def test_signal_validation_history(
    signal_repository,
    sample_signals
):
