...
class AccountDiscovery:
    ...
    def __init__(
        ...
        self.twitter_rate_limiter = RateLimiter(
            max_requests=180, time_window=900
        )  # 180 requests per 15 minutes
        self.youtube_rate_limiter = RateLimiter(
            max_requests=100, time_window=100
        ...
        self.cache_duration = timedelta(
            hours=6
        )  # Reduced from 24 hours for more frequent updates
        ...
        try:
            for account in seed_accounts:
                ...
                followers = self.twitter_client.get_followers(
                    screen_name=account, count=100  # Increased from 2
                )
    ...
    def _get_related_channels(self, channel_id: str) -> Set[str]:
        ...
        try:
            ...
            channels_response = (
                ...
                .list(id=channel_id, part="brandingSettings")
                .execute()
            )
            ...
            if channels_response["items"]:
                ...
                if featured_channels:
                    channels_info = (
                        self.youtube_client.channels()
                        .list(id=",".join(featured_channels), part="snippet")
                        .execute()
                    )
    ...
    async def _is_influential_trader(self, user) -> bool:
        ...
        activity_rate = user.statuses_count / max(
            (datetime.now() - user.created_at).days, 1
        )

        return (
            user.followers_count >= self.min_follower_count
            and engagement_rate >= self.min_engagement_rate
            and activity_rate >= 0.5
            and keyword_score  # At least one tweet every 2 days
            >= 3  # Require multiple relevant keywords
        )
    ...
    async def _calculate_trader_metrics(self, user) -> Dict:
        ...
        return {
            ...
            "engagement_rate": user.followers_count / max(user.friends_count, 1),
            "activity_rate": user.statuses_count
            / max((datetime.now() - user.created_at).days, 1),
            "verified": user.verified,
            "account_age_days": (datetime.now() - user.created_at).days,
            "avg_tweets_per_day": user.statuses_count
            / max((datetime.now() - user.created_at).days, 1),
