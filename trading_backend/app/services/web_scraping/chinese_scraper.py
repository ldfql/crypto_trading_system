    ...
    async def analyze_sentiment(self, text: str) -> Tuple[float, float]:
        ...
        try:
            ...
            (
                base_sentiment,
                base_confidence,
            ) = await self.sentiment_analyzer.analyze_text(text)
            logger.info(
                f"Base BERT sentiment: {base_sentiment}, confidence: {base_confidence}"
            )

            # Combine BERT and keyword-based sentiment with debug logging
            if bullish_score > bearish_score:
                logger.info(
                    f"Bullish signal detected. Scores - Bullish: {bullish_score}, Bearish: {bearish_score}"
                )
                ...
                logger.info(
                    f"Bearish signal detected. Scores - Bullish: {bullish_score}, Bearish: {bearish_score}"
                )
                ...
                logger.info(
                    f"Neutral signal. Equal scores - Bullish: {bullish_score}, Bearish: {bearish_score}"
                )
            ...
            confidence = min(
                0.95,
                bert_weight * base_confidence + keyword_weight * keyword_confidence,
            )
            ...
            logger.info(
                f"Chinese sentiment analysis complete. Score: {sentiment}, confidence: {confidence}"
            )
