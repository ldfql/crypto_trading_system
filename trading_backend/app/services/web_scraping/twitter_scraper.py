...
class TwitterScraper(BaseScraper):
    ...
    def __init__(
        ...
        self.api_secret = api_secret or os.getenv('TWITTER_API_SECRET', 'test_api_secret')
        self.access_token = access_token or os.getenv('TWITTER_ACCESS_TOKEN', 'test_access_token')
        self.access_token_secret = access_token_secret or os.getenv('TWITTER_ACCESS_TOKEN_SECRET', 'test_access_secret')
        ...
        self.influential_accounts = influential_accounts or [
            'CryptoCapo_', 'PeterLBrandt', 'CryptoCred',
            'cryptotwi', 'WhalePanda', 'CryptoHayes'
        ]
    ...
    async def get_influential_tweets(
        ...
        try:
            for account in self.influential_accounts[:2]:  # Limit to 2 accounts for testing
                ...
                for tweet in user_tweets:
                    ...
                    if (tweet.created_at > since_time and
                        (tweet.favorite_count + tweet.retweet_count) >= min_engagement and
                        self._is_trading_related(tweet.full_text)):
    ...
    def _is_trading_related(self, text: str) -> bool:
        ...
        keywords = [
            ...
            'trading', 'price', 'market', 'bull', 'bear',
            'long', 'short', 'position', 'leverage', 'futures',
            'support', 'resistance', 'breakout', 'breakdown',
            'analysis', 'chart', 'pattern', 'trend', 'signal'
        ]
