...
class YouTubeScraper(BaseScraper):
                ...
                "items": [
                    {
                        ...
                        "snippet": {
                            ...
                            "publishedAt": datetime.now(UTC).strftime(
                                "%Y-%m-%dT%H:%M:%SZ"
                            ),
                        },
                    }
    ...
    async def get_trading_insights(
        self, hours_ago: int = 1, min_views: int = 1000
    ) -> List[Dict]:
        ...
        try:
            for channel_name in self.trading_channels:
                try:
                    ...
                    channel_response = (
                        ...
                        .list(part="id,statistics", forUsername=channel_name)
                        .execute()
                    )
            ...
            return sorted(insights, key=lambda x: x["influence_weight"], reverse=True)
    ...
    def _calculate_influence_weight(
        self, views: int, likes: int, subscribers: int, comments: int
    ) -> float:
        ...
        try:
            ...
            subscribers = (
                int(subscribers) if isinstance(subscribers, str) else subscribers
            )
    ...
    async def get_strategy_insights(self, timeframe: str = "1h") -> Dict[str, Any]:
        ...
        try:
            ...
            for insight in insights:
                ...
                sentiment_value = (
                    1.0
                    if sentiment_result[0] == "bullish"
                    else (0.0 if sentiment_result[0] == "bearish" else 0.5)
                )
