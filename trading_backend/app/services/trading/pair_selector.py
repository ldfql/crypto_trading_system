...
class PairSelector:
    ...
    async def select_pairs(
        ...
        balance: Decimal,
        base_pairs: List[str],
        min_liquidity_score: Decimal = Decimal("1.2")  # Require 20% more than minimum volume
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def validate_pair(
        ...
        if volume_24h < self.min_volume_requirements[stage]:
            return False, f"Insufficient 24h volume ({float(volume_24h)} USDT) for account stage {stage}"

        # Check spread requirement
        if spread > self.max_spread_percentage[stage]:
            return False, f"Spread too high ({float(spread)}%) for account stage {stage}"
        ...
        if position_size is not None:
            ...
            if position_size > max_position:
                return False, f"Position size ({float(position_size)} USDT) exceeds maximum allowed ({float(max_position)} USDT) for market volume"
    ...
    async def get_recommended_pairs(
        ...
        suitable_pairs = await self.select_pairs(
            ...
            min_liquidity_score=Decimal("1.5")  # Require 50% more than minimum volume for recommendations
        )
