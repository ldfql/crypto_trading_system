...
class RealTimeAccuracyMonitor:
    ...
    def _update_accuracy_metrics(self) -> None:
        ...
        correct_predictions = sum(
            1 for p in verified_predictions if p["predicted"] == p["actual"]
        )
        ...
        if high_confidence_predictions:
            high_conf_correct = sum(
                1 for p in high_confidence_predictions if p["predicted"] == p["actual"]
            )
    ...
    def adjust_confidence_threshold(self) -> None:
        ...
        for threshold in np.arange(0.5, 1.0, 0.05):
            high_conf_preds = [
                p for p in verified_predictions if p["confidence"] >= threshold
            ]
            if high_conf_preds:
                correct = sum(
                    1 for p in high_conf_preds if p["predicted"] == p["actual"]
                )
