...
class SignalMonitor:
    ...
    async def monitor_active_signals(
        ...
        for signal in active_signals:
            ...
            account_stage = None
            if account_balance:
                account_stage = await self.account_monitor.get_account_stage(account_balance)
                # Validate position size for current account stage
                is_valid, _ = await self.account_monitor.validate_position_size(
                    symbol=signal.symbol,
                    position_size=Decimal(str(signal.position_size)) if signal.position_size else Decimal("0"),
